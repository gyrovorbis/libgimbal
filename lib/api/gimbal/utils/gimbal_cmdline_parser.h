...
typedef struct GblCmdLineParser {
    ...
    GblArrayList                   positionalArgs;
    GblArrayList                   options;
    GblArrayList                   unknownOptions;
    struct {
        const char*             pName;
        //          name, desc, version
    }                           appInfo;
...
GBL_INLINE GBL_API gblCmdLineParserConstruct(GblCmdLineParser* pParser, GblContext hCtx) {
    ...
    pParser->hCtx = hCtx;
    GBL_API_CALL(GblArrayList_construct(&pParser->positionalArgs, hCtx, sizeof(GblCmdLinePositionalArg),    NULL, 0, GBL_CMDLINE_PARSER_POSITIONAL_ARG_VECTOR_INITIAL_CAPACITY));
    GBL_API_CALL(GblArrayList_construct(&pParser->options,        hCtx, sizeof(GblCmdLineOption),           NULL, 0, GBL_CMDLINE_PARSER_OPTION_VECTOR_INITIAL_CAPACITY));
    GBL_API_CALL(GblArrayList_construct(&pParser->unknownOptions, hCtx, sizeof(GblCmdLineOption),           NULL, 0, GBL_CMDLINE_PARSER_UNKNOWN_OPTION_VECTOR_INITIAL_CAPACITY));
    GBL_API_END();
}
...
GBL_INLINE GBL_API gblCmdLineParserDestruct(GblCmdLineParser* pParser) {
    ...
    GBL_API_CALL(GblArrayList_destruct(&pParser->positionalArgs));
    GBL_API_CALL(GblArrayList_destruct(&pParser->options));
    GBL_API_CALL(GblArrayList_destruct(&pParser->unknownOptions));
    GBL_API_END();
}
...
GBL_INLINE GBL_API gblCmdLineParserPositionalArgAdd(GblCmdLineParser* pParser, const char* pName, const char* pDesc) {
    ...
    GBL_API_CALL(GblArrayList_pushBack(&pParser->positionalArgs, &arg));
    GBL_API_END();
}
...
GBL_INLINE GBL_API gblCmdLineParserOptionAdd(GblCmdLineParser* pParser, const char* pName, const char* pOptionShort, const char* pOptionLong, const char* pDesc, const char* pDefaultValue) {
    ...
    GBL_API_CALL(GblArrayList_pushBack(&pParser->options, &option));
    GBL_API_END();
}
...
GBL_INLINE GBL_API gblCmdLineParserOptionValue(const GblCmdLineParser* pParser, const char* pOptionName, const char** ppValue) {
    ...
    *ppValue = NULL;
    ...
    GBL_API_CALL(GblArrayList_size(&pParser->options, &size));
    for(GblSize o = 0; o < size; ++o) {
        GBL_API_CALL(GblArrayList_at(&pParser->options, &pOption));
...
GBL_INLINE GBL_API gblCmdLineParserPositionalArgValue(const GblCmdLineParser* pParser, uint32_t argPos, const char** ppValue) {
    ...
    *ppValue = NULL;
    ...
    GBL_API_BEGIN(pParser->hCtx);
    GBL_API_VERIFY_POINTER(ppValue);
    GBL_API_CALL(GblArrayList_size(&pParser->positionalArgs, &size));
    GBL_API_VERIFY(argPos < size, GBL_RESULT_ERROR_INVALID_CMDLINE_ARG, "Index %u is out of range!", argPos);
    GBL_API_CALL(GblArrayList_at(&pParser->positionalArgs, &pArg));
...
GBL_INLINE GBL_API gblCmdLineParserUnknownOption(const GblCmdLineParser* pParser, GblSize index, const char** ppOptionName) {
    ...
    *ppOptionName = NULL;
    GBL_API_CALL(GblArrayList_size(&pParser->unknownOptions));
    GBL_API_VERIFY(index < size, GBL_RESULT_ERROR_INVALID_CMDLINE_ARG, "Index %u is out of range!", index);
    GBL_API_CALL(GblArrayList_at(&pParser->unknownOptions, index));
...
GBL_INLINE GBL_API gblCmdLineParserParse(GblCmdLineParser* pParser, int argc, const char** ppArgv) {
    ...
    for(int a = 0; a < argc; ++a) {
        ...
        if(pArgv[0] == '-') {
            ...
            GblCmdLineOption* pOption = NULL;
            GBL_API_CALL(GblArrayList_size(&pParser->options, &optionSize));
            ...
            if(!found) {
                ...
                GBL_API_CALL(GblArrayList_pushBack(&pParser->unknownOptions, &option));
                ++unknownOptionCount;
            } else {
            ...
            GblCmdLinePositionalArg* pArg = NULL;
            GblSize size = 0;
            GBL_API_CALL(GblArrayList_size(&pParser->positionalArgs));
            if(positionalArgCount < size) {
                GBL_API_CALL(GblArrayList_at(&pParser->positionalArgs, positionalArgCount, &pArg));
                pArg->pValue = pArgv;
            } else {
