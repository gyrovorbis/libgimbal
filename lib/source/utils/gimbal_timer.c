...
GBL_EXPORT void GblTimer_stop(GblTimer* pSelf) {
...
GBL_EXPORT void GblTimer_continue(GblTimer* pSelf) {
    if(!pSelf->active) {
        const GblTimeSpec elapsed =
            GblTime_specDiff(&pSelf->stopTime, &pSelf->startTime);
...
GBL_EXPORT GblBool GblTimer_isValid(const GblTimer* pSelf) {
    return !pSelf->active &&
            pSelf->elapsedTime.tv_sec >= 0 &&
            pSelf->elapsedTime.tv_sec >= 0;
}

GBL_EXPORT uint64_t GblTimer_elapsedNs(const GblTimer* pSelf) {
...
GBL_EXPORT double GblTimer_elapsedUs(const GblTimer* pSelf) {
    return (double)GblTimer_elapsedNs(pSelf) / 1000.0;
}

GBL_EXPORT double GblTimer_elapsedMs(const GblTimer* pSelf) {
    return (double)GblTimer_elapsedNs(pSelf) / 1000000.0;
}

GBL_EXPORT double GblTimer_elapsedSecs(const GblTimer* pSelf) {
    return (double)GblTimer_elapsedNs(pSelf) /
           (double)GBL_TIME_NSECS_PER_SEC;
}
