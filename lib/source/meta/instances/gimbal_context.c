...
GBL_EXPORT void GblContext_logBuildInfo(const GblContext* pSelf) {
    ...
    GBL_CTX_PUSH();
    GBL_CTX_INFO("%-20s: %-100.100s", "Title", GBL_STRINGIFY(GBL_BUILD_CI_COMMIT_TITLE));
    GBL_CTX_INFO("%-20s: %-100.100s", "Timestamp", GBL_STRINGIFY(GBL_BUILD_CI_COMMIT_TIMESTAMP));
    GBL_CTX_INFO("%-20s: %-100.100s", "SHA", GBL_STRINGIFY(GBL_BUILD_CI_COMMIT_SHA));
    GBL_CTX_INFO("%-20s: %-100.100s", "Tag", GBL_STRINGIFY(GBL_BUILD_CI_COMMIT_TAG));
    GBL_CTX_INFO("%-20s: %-100.100s", "Ref", GBL_STRINGIFY(GBL_BUILD_CI_COMMIT_BRANCH));
    GBL_CTX_POP(1);

    GBL_CTX_INFO("Job Info");
    GBL_CTX_PUSH();
    GBL_CTX_INFO("%-20s: %-100.100s", "ID", GBL_STRINGIFY(GBL_BUILD_CI_JOB_ID));
    GBL_CTX_INFO("%-20s: %-100.100s", "Name", GBL_STRINGIFY(GBL_BUILD_CI_JOB_NAME));
    GBL_CTX_INFO("%-20s: %-100.100s", "Stage", GBL_STRINGIFY(GBL_BUILD_CI_JOB_STAGE));
    GBL_CTX_POP(1);

    GBL_CTX_INFO("User Info");
    GBL_CTX_PUSH();
    GBL_CTX_INFO("%-20s: %-100.100s", "Login", GBL_STRINGIFY(GBL_BUILD_CI_USER_LOGIN));
    GBL_CTX_INFO("%-20s: %-100.100s", "Name", GBL_STRINGIFY(GBL_BUILD_CI_USER_NAME));
    GBL_CTX_INFO("%-20s: %-100.100s", "Email Address", GBL_STRINGIFY(GBL_BUILD_CI_USER_EMAIL));
    GBL_CTX_POP(1);

    GBL_CTX_INFO("Build Node Info");
    GBL_CTX_PUSH();
    GBL_CTX_INFO("%-20s: %-100.100s", "Description", GBL_STRINGIFY(GBL_BUILD_CI_RUNNER_DESCRIPTION));
    //GBL_CTX_INFO("%-20s: %-100.100s", "Tags", GBL_STRINGIFY(GBL_BUILD_CI_RUNNER_TAGS));
    GBL_CTX_INFO("%-20s: %-100.100s", "Architecture", GBL_STRINGIFY(GBL_BUILD_CI_RUNNER_ARCH));
    GBL_CTX_POP(1);
    #else
