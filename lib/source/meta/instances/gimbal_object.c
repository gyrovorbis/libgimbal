...
static GBL_RESULT GblObjectClass_ivariantIFace_compare_(const GblVariant* pVariant, const GblVariant* pOther, int* pResult) {
    GBL_CTX_BEGIN(NULL);
...
static GBL_RESULT GblObjectClass_iTableIFace_index_(const GblITable* pITable, const GblVariant* pKey, GblVariant* pValue) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_RESULT() = GBL_RESULT_ERROR_INVALID_PROPERTY;
    GblObject* pObject = GBL_OBJECT(pITable);
    if(GblVariant_typeOf(pKey) == GBL_STRING_TYPE) {
        const char* pString = NULL;
        GBL_CTX_CALL(GblVariant_getValuePeek(pKey, &pString));
        GBL_CTX_RESULT() = GblObject_propertyVariant(pObject, pString, pValue);
        if(!GBL_RESULT_SUCCESS(GBL_CTX_RESULT()) && GBL_PRIV_REF(GBL_BOX(pObject)).derivedFlags & 0x1) {
            const GblObject* pParent = GblObject_parent(pObject);
            if(pParent) {
                GBL_CTX_CALL(GblITable_index(GBL_ITABLE(pParent), pKey, pValue));
...
static GBL_RESULT GblObjectClass_iTableIFace_newIndex_(GblITable* pITable, const GblVariant* pKey, const GblVariant* pValue) {
    ...
    if(GblVariant_typeOf(pKey) == GBL_STRING_TYPE) {
        const char* pString = NULL;
        GBL_CTX_CALL(GblVariant_getValuePeek(pKey, &pString));
        GBL_CTX_RESULT() = GblObject_setPropertyVariant(pObject, pString, (GblVariant*)pValue);
        if(!GBL_RESULT_SUCCESS(GBL_CTX_RESULT()) && GBL_PRIV_REF(GBL_BOX(pObject)).derivedFlags & 0x1) {
            const GblObject* pParent = GblObject_parent(pObject);
            if(pParent) {
                GBL_CTX_CALL(GblITable_newIndex(GBL_ITABLE(pParent), pKey, pValue));
                GBL_CTX_RESULT() = GBL_RESULT_SUCCESS;
            }
        }
    }
...
static GBL_RESULT GblObjectClass_iTableIFace_nextIndex_(const GblITable* pITable, const GblVariant* pKey, GblVariant* pNextKey, GblVariant* pNextValue) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_RESULT() = GBL_RESULT_ERROR_INVALID_PROPERTY;
    GBL_CTX_CALL(GblVariant_setValueCopy(pNextKey, GBL_NIL_TYPE, NULL));
    GBL_CTX_CALL(GblVariant_setValueCopy(pNextValue, GBL_NIL_TYPE, NULL));
    GblObject* pObject = GBL_OBJECT(pITable);
    // get the first one
    if(!pKey || GblVariant_typeOf(pKey) == GBL_NIL_TYPE) {
        ...
        if(pProp) {
            GBL_CTX_CALL(GblVariant_setValueCopy(pNextKey, GBL_STRING_TYPE, GblProperty_nameString(pProp)));
            GBL_CTX_CALL(GblObject_propertyVariant(pObject, GblProperty_nameString(pProp), pNextValue));
            GBL_CTX_RESULT() = GBL_RESULT_SUCCESS;
        }
...
GBL_EXPORT GBL_RESULT GblObject_propertyVaList_(const GblObject* pSelf, const GblProperty* pProp, va_list* pList) {
    ...
    GBL_CTX_CALL(GblObject_property_(pSelf, pProp, &variant));
...
GBL_EXPORT GBL_RESULT GblObject_constructVaListWithClass(GblObject* pSelf, GblObjectClass* pClass, va_list* pVarArgs) {
    ...
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_TYPE(type, GBL_OBJECT_TYPE);
    GBL_CTX_CALL(GblBox_constructWithClass((GblBox*)pSelf, GBL_BOX_CLASS(pClass)));
    GBL_CTX_CALL(GblObject_constructVaList_(pSelf, type, pVarArgs));
    GBL_CTX_END();
}
...
GBL_EXPORT GblObject* GblObject_findAncestorByType(const GblObject* pSelf, GblType ancestorType) {
    ...
    //GBL_CTX_BEGIN(GblType_context(GBL_INSTANCE_TYPEOF(pSelf))); {
