...
GBL_EXPORT GBL_RESULT GblIVariantClass_constructDefault(const GblIVariantClass* pSelf, GblVariant* pVariant) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
    GBL_CTX_VERIFY_POINTER(pVariant);
    if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_CONSTRUCT_DEFAULT) {
        GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnConstruct);
        GBL_CTX_CALL(pSelf->pVTable->pFnConstruct(pVariant, 0, NULL, GBL_IVARIANT_OP_FLAG_CONSTRUCT_DEFAULT));
    }
    GBL_CTX_END();
}
 ...
 * 3) Shit pants
 */
GBL_EXPORT GBL_RESULT GblIVariantClass_constructCopy(const GblIVariantClass* pSelf, GblVariant* pVariant, const GblVariant* pOther) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
    GBL_CTX_VERIFY_POINTER(pVariant);
    ...
    if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_CONSTRUCT_COPY) {
        GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnConstruct);
        GBL_CTX_CALL(pSelf->pVTable->pFnConstruct(pVariant, 1, (GblVariant*)pOther, GBL_IVARIANT_OP_FLAG_CONSTRUCT_COPY));
        ...
        GBL_CTX_CALL(pSelf->pVTable->pFnSet(pVariant, 1, (GblVariant*)pOther, GBL_IVARIANT_OP_FLAG_SET_COPY));
 ...
 * 6) Shit pants
 */
GBL_EXPORT GBL_RESULT GblIVariantClass_constructMove(const GblIVariantClass* pSelf, GblVariant* pVariant, GblVariant* pOther) {
   GBL_CTX_BEGIN(NULL);
   GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
   GBL_CTX_VERIFY_POINTER(pVariant);
   GBL_CTX_VERIFY_POINTER(pOther);
   if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_CONSTRUCT_MOVE) {
       GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnConstruct);
       GBL_CTX_CALL(pSelf->pVTable->pFnConstruct(pVariant, 1, pOther, GBL_IVARIANT_OP_FLAG_CONSTRUCT_MOVE));
       ...
       GBL_CTX_CALL(pSelf->pVTable->pFnSet(pVariant, 1, pOther, GBL_IVARIANT_OP_FLAG_SET_MOVE));
   } else if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_CONSTRUCT_COPY) {
        GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnConstruct);
        GBL_CTX_CALL(pSelf->pVTable->pFnConstruct(pVariant, 1, pOther, GBL_IVARIANT_OP_FLAG_CONSTRUCT_COPY));
   } else if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_SET_COPY) {
       GBL_CTX_VERIFY_EXPRESSION(pSelf->pVTable->pFnSet);
       if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_CONSTRUCT_DEFAULT) {
            GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnConstruct);
            GBL_CTX_CALL(pSelf->pVTable->pFnConstruct(pVariant, 0, NULL, GBL_IVARIANT_OP_FLAG_CONSTRUCT_DEFAULT));
       }
       GBL_CTX_CALL(pSelf->pVTable->pFnSet(pVariant, 1, pOther, GBL_IVARIANT_OP_FLAG_SET_COPY));
   } else if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_RELOCATABLE) {
       memcpy(pVariant, pOther, sizeof(GblVariant));
   } else {
       GBL_CTX_RECORD_SET(GBL_RESULT_ERROR_INVALID_OPERATION,
...
GBL_EXPORT GBL_RESULT GblIVariantClass_setCopy(const GblIVariantClass* pSelf, GblVariant* pVariant, const GblVariant* pOther) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
    GBL_CTX_VERIFY_POINTER(pVariant);
    GBL_CTX_VERIFY_POINTER(pOther);
 ...
 * 4) Shit pants
 */
GBL_EXPORT GBL_RESULT GblIVariantClass_setMove(const GblIVariantClass* pSelf, GblVariant* pVariant, GblVariant* pOther) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
    GBL_CTX_VERIFY_POINTER(pVariant);
    GBL_CTX_VERIFY_POINTER(pOther);
...
GBL_EXPORT GBL_RESULT GblIVariantClass_destruct(const GblIVariantClass* pSelf, GblVariant* pVariant) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
    GBL_CTX_VERIFY_POINTER(pVariant);
    if(pSelf->pVTable->pFnDestruct) {
        GBL_CTX_CALL(pSelf->pVTable->pFnDestruct(pVariant));
    }
    GBL_CTX_END();
}

GBL_EXPORT GBL_RESULT GblIVariantClass_compare(const GblIVariantClass* pSelf, const GblVariant* pVariant, const GblVariant* pOther, int* pCmpResult) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
    GBL_CTX_VERIFY_POINTER(pVariant);
    GBL_CTX_VERIFY_POINTER(pOther);
    GBL_CTX_VERIFY_POINTER(pCmpResult);
    if(pVariant->type != pOther->type) {
        *pCmpResult = INT_MAX;
    } else if(pSelf->pVTable->pFnCompare) {
        GBL_CTX_CALL(pSelf->pVTable->pFnCompare(pVariant, pOther, pCmpResult));
...
static GBL_RESULT GblIVariantClass_valuesFromVa_(const GblIVariantClass* pSelf, const char* pFmt, va_list* pArgList, size_t * pCount, GblVariant pVariant[]) {
    ...
    *pCount = 0;
    ...
    while((curChar = pFmt[*pCount]) != '\0') {
        ...
        default: GBL_CTX_ERROR("[GblIVariant] Unknown va value type: %c", curChar); break;
        }
...
GBL_EXPORT GBL_RESULT GblIVariantClass_constructValueCopy(const GblIVariantClass* pSelf, GblVariant* pVariant, va_list* pArgs) {
    GBL_CTX_BEGIN(NULL); {
        ...
        if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_CONSTRUCT_VALUE_COPY) {
            GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnConstruct);
            GBL_CTX_CALL(pSelf->pVTable->pFnConstruct(pVariant, count, pVariants, GBL_IVARIANT_OP_FLAG_CONSTRUCT_VALUE_COPY));
            ...
            GBL_CTX_CALL(pSelf->pVTable->pFnSet(pVariant, count, pVariants, GBL_IVARIANT_OP_FLAG_SET_VALUE_COPY));
        } else {
            GBL_CTX_RECORD_SET(GBL_RESULT_ERROR_INVALID_OPERATION,
...
GBL_EXPORT GBL_RESULT GblIVariantClass_constructValueMove(const GblIVariantClass* pSelf, GblVariant* pVariant, va_list* pArgs) {
    GBL_CTX_BEGIN(NULL); {
        ...
        if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_CONSTRUCT_VALUE_MOVE) {
            GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnConstruct);
            GBL_CTX_CALL(pSelf->pVTable->pFnConstruct(pVariant, count, pVariants, GBL_IVARIANT_OP_FLAG_CONSTRUCT_VALUE_MOVE));
            ...
            GBL_CTX_CALL(pSelf->pVTable->pFnSet(pVariant, count, pVariants, GBL_IVARIANT_OP_FLAG_SET_VALUE_MOVE));
...
GBL_EXPORT GBL_RESULT GblIVariantClass_setValueCopy(const GblIVariantClass* pSelf, GblVariant* pVariant, va_list* pArgs) {
    GBL_CTX_BEGIN(NULL); {
        GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
        size_t  count = 0;
        GblVariant* pVariants = GBL_ALLOCA(sizeof(GblVariant)*GBL_IVARIANT_VALUE_VAR_ARG_MAX);
...
GBL_EXPORT GBL_RESULT GblIVariantClass_setValueMove(const GblIVariantClass* pSelf, GblVariant* pVariant, va_list* pArgs) {
    GBL_CTX_BEGIN(NULL); {
        GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
        size_t  count = 0;
        GblVariant* pVariants = GBL_ALLOCA(sizeof(GblVariant)*GBL_IVARIANT_VALUE_VAR_ARG_MAX);
...
GBL_EXPORT GBL_RESULT GblIVariantClass_getValueCopy(const GblIVariantClass* pSelf, const GblVariant* pVariant, va_list* pArgs) {
    GBL_CTX_BEGIN(NULL); {
        GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
        size_t  count = 0;
        GblVariant* pVariants = GBL_ALLOCA(sizeof(GblVariant)*GBL_IVARIANT_VALUE_VAR_ARG_MAX);
        ...
        if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_GET_VALUE_COPY) {
            GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnGet);
            GBL_CTX_CALL(pSelf->pVTable->pFnGet((GblVariant*)pVariant, count, pVariants, GBL_IVARIANT_OP_FLAG_GET_VALUE_COPY));
        } else {
            GBL_CTX_RECORD_SET(GBL_RESULT_ERROR_INVALID_OPERATION,
...
GBL_EXPORT GBL_RESULT GblIVariantClass_getValuePeek(const GblIVariantClass* pSelf, const GblVariant* pVariant, va_list* pArgs) {
    GBL_CTX_BEGIN(NULL); {
        GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
        size_t  count = 0;
        GblVariant* pVariants = GBL_ALLOCA(sizeof(GblVariant)*GBL_IVARIANT_VALUE_VAR_ARG_MAX);
        ...
        if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_GET_VALUE_PEEK) {
            GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnGet);
            GBL_CTX_CALL(pSelf->pVTable->pFnGet((GblVariant*)pVariant, count, pVariants, GBL_IVARIANT_OP_FLAG_GET_VALUE_PEEK));
        } else {
            GBL_CTX_RECORD_SET(GBL_RESULT_ERROR_INVALID_OPERATION,
...
GBL_EXPORT GBL_RESULT GblIVariantClass_getValueMove(const GblIVariantClass* pSelf, const GblVariant* pVariant, va_list* pArgs) {
    GBL_CTX_BEGIN(NULL); {
        GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
        size_t  count = 0;
        GblVariant* pVariants = GBL_ALLOCA(sizeof(GblVariant)*GBL_IVARIANT_VALUE_VAR_ARG_MAX);
        ...
        if(pSelf->pVTable->supportedOps & GBL_IVARIANT_OP_FLAG_GET_VALUE_MOVE) {
            GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnGet);
            GBL_CTX_CALL(pSelf->pVTable->pFnGet((GblVariant*)pVariant, count, pVariants, GBL_IVARIANT_OP_FLAG_GET_VALUE_MOVE));
...
GBL_EXPORT GBL_RESULT GblIVariantClass_save(const GblIVariantClass* pSelf, const GblVariant* pVariant, GblStringBuffer* pString) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
    GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnSave);
    GBL_CTX_CALL(pSelf->pVTable->pFnSave(pVariant, pString));
    GBL_CTX_END();
}
GBL_EXPORT GBL_RESULT GblIVariantClass_load(const GblIVariantClass* pSelf, GblVariant* pVariant, const GblStringBuffer* pString) {
    GBL_CTX_BEGIN(NULL);
    GBL_CTX_VERIFY_POINTER(pSelf && pSelf->pVTable);
    GBL_CTX_VERIFY_POINTER(pSelf->pVTable->pFnLoad);
    GBL_CTX_CALL(pSelf->pVTable->pFnLoad(pVariant, pString));
    GBL_CTX_END();
}
