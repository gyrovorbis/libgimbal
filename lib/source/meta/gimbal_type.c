...
GBL_API gblTypeReinit(GblContext* pCtx) {
    GBL_API_BEGIN(NULL);
    if(!initialized_) {
...
GBL_EXPORT GBL_RESULT gblTypeClassConstructFloating(GblType type, GblClass* pClass) {
    GBL_API_BEGIN(pCtx_);
    GBL_API_VERIFY_ARG(type != GBL_TYPE_INVALID);
    GBL_API_VERIFY_POINTER(pClass);
    gblTypeClassRef(type); // have to reference actual type
    GBL_API_CALL(gblTypeClassConstruct_(pClass, GBL_META_CLASS(type)));
    GBL_API_END();
}
...
static GblClass* gblTypeClassCreate_(GblMetaClass* pMeta, GblBool floating) {
    ...
    if(!*ppClass) {
        ...
        *ppClass = GBL_API_MALLOC(pMeta->info.classSize, pMeta->info.classAlign, pMeta->pName);
    ...
    // Call constructor
    GBL_API_CALL(gblTypeClassConstruct_(*ppClass, pMeta));
...
GBL_EXPORT GblClass* gblTypeClassCreateFloating(GblType type) {
    GblClass* pClass = NULL;
    GBL_API_BEGIN(pCtx_);
    GBL_API_VERIFY_ARG(type != GBL_TYPE_INVALID);
    gblTypeClassRef(type);
    pClass = gblTypeClassCreate_(GBL_META_CLASS(type), GBL_TRUE);
    GBL_API_END_BLOCK();
    return pClass;
}
...
GBL_EXPORT GBL_RESULT gblTypeClassDestructFloating(GblClass* pClass) {
    GBL_API_BEGIN(pCtx_);
    GBL_API_VERIFY_POINTER(pClass);
    const GblType type = GBL_CLASS_TYPE(pClass);
    GBL_API_CALL(gblTypeClassDestruct_(pClass));
    gblTypeClassUnref(gblTypeClassPeek(type));
    GBL_API_END();
}

GBL_EXPORT GBL_RESULT gblTypeClassDestroyFloating(GblClass* pClass) {
    GBL_API_BEGIN(pCtx_);
    GBL_API_VERIFY_POINTER(pClass);
    GBL_API_CALL(gblTypeClassDestructFloating(pClass));
    GBL_API_FREE(pClass);
    GBL_API_END();
}
...
GBL_EXPORT GBL_RESULT gblTypeInstanceConstruct_(GblType type, GblInstance* pInstance, GblClass* pClass) {
    GblMetaClass*   pMeta = (GblMetaClass*)type;
    GBL_API_BEGIN(pCtx_);
    ...
    // Set Class
    if(!pClass) pClass = gblTypeClassRef(type);
    else gblTypeClassRef(type);
...
GBL_EXPORT GBL_RESULT gblTypeInstanceConstruct(GblType type, GblInstance* pInstance) {
    GBL_API_BEGIN(pCtx_);
    GBL_API_CALL(gblTypeInstanceConstruct_(type, pInstance, NULL));
    GBL_API_END();
}

GBL_EXPORT GBL_RESULT gblTypeInstanceConstructWithClass(GblInstance* pInstance, GblClass* pClass) {
    GBL_API_BEGIN(pCtx_);
    GBL_API_VERIFY_POINTER(pClass);
    GBL_API_CALL(gblTypeInstanceConstruct_(GBL_CLASS_TYPE(pClass), pInstance, pClass));
    GBL_API_END();
}

GBL_EXPORT GblInstance* gblTypeInstanceCreate_(GblType type, GblClass* pClass) {
    ...
    GBL_API_CALL(gblTypeInstanceConstruct_(type, pInstance, pClass));
...
GBL_EXPORT GblInstance* gblTypeInstanceCreate(GblType type) {
    GblInstance* pInstance = NULL;
    GBL_API_BEGIN(pCtx_);
    pInstance = gblTypeInstanceCreate_(type, NULL);
    GBL_API_END_BLOCK();
    return pInstance;
}

GBL_EXPORT GblInstance* gblTypeInstanceCreateWithClass(GblClass* pClass) {
    GblInstance* pInstance = NULL;
    GBL_API_BEGIN(pCtx_);
    pInstance = gblTypeInstanceCreate_(GBL_CLASS_TYPE(pClass), pClass);
    GBL_API_END_BLOCK();
    return pInstance;
}
...
GBL_EXPORT GblRefCount gblTypeInstanceDestruct(GblInstance* pInstance) {
    GblRefCount refCount = 0;
    GBL_API_BEGIN(pCtx_);
    if(gblTypeClassIsFloating(gblTypeClassFromInstance(pInstance))) {
        //gblTypeClassDestroyFloating(gblTypeClassFromInstance(pInstance));
    } else {
...
GBL_EXPORT GblBool gblTypeClassIsFloating(const GblClass* pClass) {
    GblBool floating = GBL_FALSE;
    GBL_API_BEGIN(pCtx_);
    if(pClass) {
       GblMetaClass* pMeta = GBL_META_CLASS(pClass->type);
       if(!pMeta->pClass || pMeta->pClass != pClass)
           floating = GBL_TRUE;
    }
    GBL_API_END_BLOCK();
    return floating;
...
GBL_EXPORT GblBool gblTypeClassCheck(const GblClass* pClass, GblType type) {
    GblBool result = GBL_FALSE;
    GBL_API_BEGIN(pCtx_);
...
GBL_EXPORT GblBool gblTypeInstanceCheck(const GblInstance* pInstance, GblType type) {
    GblBool result = GBL_FALSE;
    GBL_API_BEGIN(pCtx_);
    if(!(!pInstance && type == GBL_TYPE_INVALID)) {
        GBL_API_VERIFY_POINTER(pInstance);
        result = gblTypeClassCheck(pInstance->pClass, type);
    }
    GBL_API_END_BLOCK();
    return result;
}
...
GBL_EXPORT GblInstance* typeInstanceCast_(const GblInstance* pInstance, GblType type, GblBool check) {
    ...
    if(!(!pInstance && type == GBL_TYPE_INVALID)) {
        GBL_API_VERIFY_POINTER(pInstance);
        if(gblTypeInstanceCheck(pInstance, type)) {
...
GBL_EXPORT GBL_RESULT gblTypeInstanceClassSwizzle(GblInstance* pInstance, GblClass* pClass) GBL_NOEXCEPT {
    ...
    GBL_API_VERIFY_POINTER(pInstance);
    GBL_API_VERIFY_POINTER(pClass);
    GblClass* pClassOld = GBL_INSTANCE_GET_CLASS(pInstance);

    // If we're replacing an existing class
    if(pClassOld) {
        // ensure that we're the same or a derived type
        if(!gblTypeIsA(GBL_CLASS_TYPE(pClass), GBL_CLASS_TYPE(pClassOld))) {
            GBL_API_RECORD_SET(GBL_RESULT_ERROR_INVALID_TYPE,
                               "[GblType] Cannot swizzle from class type [%s] to unrelated class type [%s].",
                               gblTypeName(GBL_CLASS_TYPE(pClassOld)), gblTypeName(GBL_CLASS_TYPE(pClass)));
        }

        // destroy or unreference
        if(gblTypeClassIsFloating(pClassOld)) {
            GBL_API_CALL(gblTypeClassDestroyFloating(pClassOld));
        }
    }
