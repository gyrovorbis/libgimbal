#include "types/test_gimbal_ref.h"
...
GBL_API GblRef_test_alloc(GblContext* pCtx) {
    ...
    GBL_TEST_VERIFY(pRefs_[0]);
    GBL_TEST_COMPARE(GblRef_activeCount(), 1);
    GBL_TEST_COMPARE(GblRef_refCount(pRefs_[0]), 1);
    GBL_TEST_COMPARE(GblRef_context(pRefs_[0]), NULL);
    GBL_API_END();
}
GBL_API GblRef_test_allocWithContext(GblContext* pCtx) {
    ...
    GBL_API_VERIFY_LAST_RECORD();
    GBL_TEST_VERIFY(pRefs_[1]);
    GBL_TEST_COMPARE(GblRef_activeCount(), 2);
    GBL_TEST_COMPARE(GblRef_refCount(pRefs_[1]), 1);
    GBL_TEST_COMPARE(GblRef_context(pRefs_[1]), pCtx);
    GBL_API_END();
}
GBL_API GblRef_test_acquire(GblContext* pCtx) {
    ...
    GBL_API_VERIFY_LAST_RECORD();
    GBL_TEST_COMPARE(pRefs_[2], pRefs_[0]);
    GBL_TEST_COMPARE(strncmp(pRefs_[2], "String1", 256), 0);
    GBL_TEST_COMPARE(GblRef_refCount(pRefs_[0]), 2);
    GBL_TEST_COMPARE(GblRef_activeCount(), 2);

    pRefs_[3] = GblRef_acquire(pRefs_[1]);
    GBL_API_VERIFY_LAST_RECORD();
    GBL_TEST_COMPARE(pRefs_[3], pRefs_[1]);
    GBL_TEST_COMPARE(strncmp(pRefs_[3], "String2", 256), 0);
    GBL_TEST_COMPARE(GblRef_refCount(pRefs_[1]), 2);
    GBL_TEST_COMPARE(GblRef_activeCount(), 2);
...
GBL_API GblRef_test_destructFail(GblContext* pCtx) {
    ...
    GBL_TEST_COMPARE(strncmp(pRefs_[0], "String1", 256), 0);
    GBL_API_END();
}
