#include "strings/test_gimbal_string_ref.h"
...
GBL_API GblStringRef_test_null(GblContext* pCtx) {
    ...
    GBL_TEST_COMPARE(GblStringRef_at(pRefs_[0], 0), '\0');
...
GBL_API GblStringRef_test_empty_at(GblContext* pCtx) {
    ...
    GBL_TEST_COMPARE(GblStringRef_at(pRefs_[0], 1), '\0');
    GBL_TEST_COMPARE(GBL_API_LAST_RESULT(), GBL_RESULT_ERROR_OUT_OF_RANGE);
    GBL_API_END();
}
...
GBL_API GblStringRef_test_acquire(GblContext* pCtx) {
    ...
    GBL_TEST_COMPARE(pRefs_[6], pRefs_[5]);
    GBL_API_END();
}
...
GBL_API GblStringRef_test_final(GblContext* pCtx) {
    GBL_API_BEGIN(pCtx);
    GBL_TEST_COMPARE(GblRef_activeCount(), beginActiveRefCount_);
    GBL_API_END();
}
