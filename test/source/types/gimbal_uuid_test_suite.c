...
static GBL_RESULT GblUuidTestSuite_initNil_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    GBL_API_VERIFY_CALL(GblUuid_initNil(&uuid1));
    GBL_API_VERIFY_CALL(GblUuid_initNil(&uuid2));
...
static GBL_RESULT GblUuidTestSuite_initFromString_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    for(int u = 0; u < GBL_ARRAY_SIZE(uuidStrings); ++u) {
        GBL_API_VERIFY_CALL(GblUuid_initFromString(&uuids[u], uuidStrings[u]));
        ...
        GblUuid tempUuid;
        GBL_API_VERIFY_CALL(GblUuid_initFromString(&tempUuid, uuidStrings[u]));
...
static GBL_RESULT GblUuidTestSuite_toString_(GblTestSuite* pSelf, GblContext* pCtx) {
    ...
    for(int u = 0; u < GBL_ARRAY_SIZE(uuidStrings); ++u) {
        GBL_API_VERIFY_CALL(GblUuid_initFromString(&uuids[u], uuidStrings[u]));
        GBL_API_VERIFY_CALL(GblUuid_toString(&uuids[u], uuidStringBuffer));
        GBL_TEST_COMPARE(uuidStrings[u], uuidStringBuffer);

        GblUuid tempUuid;
        GBL_API_VERIFY_CALL(GblUuid_initFromString(&tempUuid, uuidStringBuffer));
        GBL_TEST_COMPARE(GblUuid_compare(&uuids[u], &tempUuid), 0);
