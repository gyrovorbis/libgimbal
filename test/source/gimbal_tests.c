...
static GBL_RESULT GimbalTests_constructed_(GblObject* pSelf) {
    GBL_CTX_BEGIN(NULL);

    GBL_VCALL_DEFAULT(GblObject, pFnConstructed, pSelf);

    GblObject_setName(pSelf, "libGimbalTests");
    ...
    GblTestScenario_enqueueSuite(pScenario,
                                 GblTestSuite_create(GBL_MD5_TEST_SUITE_TYPE));
    GblTestScenario_enqueueSuite(pScenario,
...
static GBL_RESULT GimbalTestsClass_init_(GblClass* pClass, const void* pUd) {
    GBL_UNUSED(pUd);
    GBL_CTX_BEGIN(NULL);
    GBL_OBJECT_CLASS(pClass)->pFnConstructed = GimbalTests_constructed_;
    GBL_CTX_END();
}

GBL_EXPORT GblType GimbalTests_type(void) {
    static GblType type = GBL_INVALID_TYPE;
    static GblTypeInfo info = {
        .pFnClassInit = GimbalTestsClass_init_,
        .classSize    = sizeof(GimbalTestsClass),
        .instanceSize = sizeof(GimbalTests),
    };
    if GBL_UNLIKELY(type == GBL_INVALID_TYPE) {
        type = GblType_register(GblQuark_internStatic("GimbalTests"),
