...
GBL_API GblOpaque_test_variant(GblContext* pCtx) {
    ...
    GBL_VERIFY(GblVariant_type(&v) == opaqueType_);
    GBL_COMPARE(GblVariant_getPointer(&v), NULL);
    ...
    GBL_COMPARE(strcmp(GblVariant_getPointer(&v), "Hi"), 0); // Utility / Get Value
    GBL_API_VERIFY_CALL(GblVariant_destruct(&v));

    // Value Move Constructor
    GBL_API_VERIFY_CALL(GblVariant_constructValueMove(&v, opaqueType_, "Bye"));
    GBL_COMPARE(strcmp(GblVariant_getPointer(&v), "Bye"), 0); // Utility / Get Value

    // Utility / Value Set Copy
    GBL_API_VERIFY_CALL(GblVariant_setOpaque(&v, opaqueType_, "Shy"));
    GBL_COMPARE(strcmp(GblVariant_getPointer(&v), "Shy"), 0); // Utility / Get Value

    // Value Set Move
    GBL_API_VERIFY_CALL(GblVariant_setValueMove(&v, opaqueType_, "Fly"));
    GBL_COMPARE(strcmp(GblVariant_getPointer(&v), "Fly"), 0); // Utility / Get Value
    ...
    GBL_COMPARE(refCountBegin, GblType_classRefCount(opaqueType_));
    GBL_API_END();
}
...
GBL_API GblOpaque_test_conversions(GblContext* pCtx) {
    ...
    GBL_VERIFY(GblVariant_canConvert(opaqueType_, GBL_POINTER_TYPE));
    ...
    GBL_VERIFY(!GblVariant_canConvert(GBL_POINTER_TYPE, opaqueType_));

    GBL_VERIFY(GblVariant_canConvert(opaqueType_, GBL_STRING_TYPE));
...
GBL_API GblOpaque_test_final(GblContext* pCtx) {
    GBL_API_BEGIN(pCtx);
    GBL_COMPARE(GblType_classRefCount(opaqueType_), 0);
    GBL_API_END();
}
